...
class v6c_mlPaymentGateway extends v6c_mlPaymentGateway_parent
{
    ...
    * @var int
    */
    const V6C_ML_PAYPAL_NVP_VER = 63;
...
//next Ordernr by eComStyle.de:
    public function getNextv6c()
    {
        $oxVersion = oxConfig::getInstance()->getActiveShop()->oxshops__oxversion->value;
        if(version_compare($oxVersion, '4.6.11', '<=')) {
        $sQ = 'select max(cast(oxorder.oxordernr as unsigned)) from oxorder where oxorder.oxshopid = "'.$this->getConfig()->getShopId().'" ';
        } else {
        $sQ = 'select max(cast(oxcounters.oxcount as unsigned)) from oxcounters where oxcounters.oxident = "oxorder" ';
        }
    ...
    */
    public function v6cInitPayment(oxPayment $oPayment, oxBasket $oBasket)
    {
        ...
        if (strcmp($oPayment->oxpayments__v6link->value, 'v6c_paypalxpr') == 0)
        {
            ...
            if ($this->getConfig()->getConfigParam('v6c_Logotyp'))
            {$aQuery['HDRIMG'] =  $this->getConfig()->getConfigParam('v6c_Logourl');}
            else
            {$aQuery['LOGOIMG'] =  $this->getConfig()->getConfigParam('v6c_Logourl');}
            $aQuery['CARTBORDERCOLOR'] = $this->getConfig()->getConfigParam('v6c_Color');
...
if ($this->getConfig()->getConfigParam('v6c_altcart')){
//BOF New Cart by PBT:
// Check if B2B-Mode by eComStyle.de:
    if ( $oBasket->isCalculationModeNetto() ){
        define("orderShowTax",1);
    } else {
        define("orderShowTax",0);
            ...
            foreach ($oBasket->getContents() as $oBasketItem)
            {
				...
				$aQuery['L_NUMBER'.$iLineNr] 	= $oProduct->oxarticles__oxartnum->rawValue;
		        $aQuery['L_QTY'.$iLineNr] 		= $iQty;
				//$aQuery['L_DESC'.$iLineNr] 		= $oProduct->oxarticles__oxean->rawValue;
			...
			$iVoucher 			= $oBasket->getVoucherDiscount()->getBruttoPrice();
			...
			if($iVoucher != 0) {
				...
				$aQuery['L_NAME'.$iLineNr] 		= oxLang::getInstance()->translateString("COUPON");
        		$aQuery['L_AMT'.$iLineNr] 		= number_format($iVoucher, 2);
    		    $aQuery['L_NUMBER'.$iLineNr] 	= "";
		        $aQuery['L_QTY'.$iLineNr] 		= 1;
				$aQuery['L_DESC'.$iLineNr] 		= "";
            ...
            //nextordernr
            $nextordernr = $this->getNextv6c();
            $aQuery['DESC'] = oxLang::getInstance()->translateString('ECS_PPYOURORDER').' '.$nextordernr.' '.oxLang::getInstance()->translateString('ECS_PPYOURORDERAT').' '.oxConfig::getInstance()->getActiveShop()->oxshops__oxname->value;
...
//BOF Original Cart by 6vCommerce:
            // Generate basket specific portion of request string
            $dBillAmt = doubleval(0);
            $i = 0;
            $sDesc = '';
            ...
            */
            $nextordernr = $this->getNextv6c();
            $sDesc = (oxLang::getInstance()->translateString('ECS_PPYOURORDER').' '.$nextordernr.' '.oxLang::getInstance()->translateString('ECS_PPYOURORDERAT').' '.oxConfig::getInstance()->getActiveShop()->oxshops__oxname->value);
...
//EOF Original Cart by 6vCommerce
        return $res;
    }
